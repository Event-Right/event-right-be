...
app.get('/dogs', async (req, res) => {
  try {
    const location = req.query.location || 'portland';
    const sort_by = req.query.sort_by || 'best_match';
    const businessList = await request
      .get(
        `https://api.yelp.com/v3/businesses/search?location=${location}&sort_by=${sort_by}&term=hotdog`
      )
...
`,
      [
        // might be nice to destructure these from the object, but you'd still end up with duplication since you need to put them in the array in a particular order, so this is verbose but fine
        req.body.name,
        req.body.categories,
