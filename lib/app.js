...
app.get('/dogs', async(req, res) => {
  try {
    ...
    const businessList = await request.get(`https://api.yelp.com/v3/businesses/search?location=${location}&term=Hot Dog`)
...
app.get('/dogs/:id', async(req, res) => {
  try { 
    ...
    const businessList = await request.get(`https://api.yelp.com/v3/businesses/${id}`)
      .set('Authorization', `Bearer ${process.env.YELP_API}`)
      .set('Accept', 'application/json');
